<p align="center">
  ...
  路
  <a href="runtime-api.zh.md">涓枃</a>
  路
  <a href="runtime-api.ja.md">鏃ユ湰瑾?/a>
</p>
...
**Documentation:** [Quick Start](quickstart.md) 路 [README](../README.md) 路 [Examples](../examples/README.md) 路 [mooncakes.io](https://mooncakes.io/docs/Mairzzcllo/moonbit_sqlc_plugin) 路 [GitHub](https://github.com/Mairzzcllo/MoonBit-sqlc-WASM-plugin)
...
Generated files already contain `import "Mairzzcllo/moonbit_sqlc_plugin/runtime"`. See [Quick Start 鈥?Path A](quickstart.md#path-a--install-runtime-from-mooncakesio) 路 [涓枃](quickstart.zh.md) 路 [鏃ユ湰瑾瀅(quickstart.ja.md) for full setup.
...
Transaction handle. Shares the same `exec`/`execrows`/`query`/`query_row` interface as `DB`, plus `commit` and `rollback`. Note: `copyfrom`, `batch`, and `execlastid` are **not** available on `Transaction` 鈥?generated code only produces `db: DB` variants for those commands (per `supports_transaction()` logic).
...
| `check_bounds(index)` | `Result[Int, DBError]` | Validate index within bounds; OOB 鈫?`Err(TypeError(...))` |
| `index_of(name)` | `Result[Int, DBError]` | Find column index by name (O(n) scan); not found 鈫?`Err(ColumnNotFound(...))` |
...
| `get_json(index)` | `Result[Json, DBError]` | JSON string 鈫?`@json.parse` |
...
| `get_time(index)` | `Result[Time, DBError]` | Time with variable-precision fractional seconds (0鈥? digits) |
...
Each returns `Result[Option[Array[T]], DBError]` 鈥?`None` if the column is NULL:
...
| `collect_limited` | `(Int) -> Result[Array[Row], DBError]` | Collect up to `max_rows` rows; exceeds 鈫?`Err(TooManyRows(count, query))` |
...
SQL TIME value decomposed into components. Variable-precision fractional seconds (0鈥? digits) are normalized by `Row::get_time`.
...
SQL TIMETZ value 鈥?time with timezone offset in seconds (e.g., +8h = `28800`, -5h = `-18000`).
...
| `MockDB::default_ok()` | `MockDB` | Quick constructor: all ops 鈫?`Ok(0L)` / empty results; `begin` 鈫?`Err` |
...
| Resource | Link |
|----------|------|
| Quick Start | [quickstart.md](quickstart.md) 路 [涓枃](quickstart.zh.md) 路 [鏃ユ湰瑾瀅(quickstart.ja.md) |
